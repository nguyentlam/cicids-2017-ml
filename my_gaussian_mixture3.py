...
class MyGaussianMixture3(BaseEstimator):
  ...
  def fit(self, X, y):
    ...
    for c in range(self.n_classes):
      ...
      if isinstance(self.n_components, list):
        means_c = get_initial_means(X_c, self.n_components[c])
      else:
  ...
  def predict_proba(self, X):
    ...
    for i, ye in enumerate(y_pre):
      new_arr = [] # array of probability for each class 
      if isinstance(self.n_components, list):
        base = 0
        for _, n_component in enumerate(self.n_components):
            new_arr.append(np.sum(ye[base: base + n_component]))
            base += n_component
      else:
